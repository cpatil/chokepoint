#!/bin/bash
# Runs whenever a volume appears or disappears, and does whatever Chokepoint has been
# asked to do about memory cards.
#
# Two independent jobs, each switched on from Chokepoint's Cards menu:
#
...
#
#   LaunchOnCardInsert   open Chokepoint, so the card's transfer is recorded from the
...
[ -d "$APP" ] || APP="/Applications/Chokepoint.app"
...
while IFS= read -r volume; do
    ...
    if [ "$LAUNCH" = "1" ]; then
        # Harmless when it is already up: Chokepoint allows one instance, so this brings
        # the running copy forward instead of starting a second.
        /usr/bin/open "$APP" 2>/dev/null && log "opened Chokepoint for $volume"
    fi
done
