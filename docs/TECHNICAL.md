# Chokepoint — technical notes

How the numbers are obtained, what they mean, where they stop being reliable, and how
the app is built and tested. The [README](../README.md) covers what it is for.
...
- Wi-Fi's reported link rate is not usable as a ceiling — mine claimed 304 Mbit/s
  ...
  the session's best instead, and Chokepoint does not try to name a Wi-Fi generation from
...
**Active first is held, not recomputed.** It is worked out when Chokepoint starts and then
...
alternative explanations Chokepoint cannot rule out.
...
Two things Chokepoint can do by itself when a memory card is inserted, both off until you
turn them on:

- **Open Chokepoint When a Card Is Inserted** — so a transfer is recorded from the first
...
Chokepoint reports the byte counts as fact and the causes as things to check, because it
observes that writes happened, not who issued them.

Sessions are kept in `~/Library/Application Support/Chokepoint/history.json` — up to 500
...
Chokepoint needs no permission for anything it measures. The single optional one is
removable volumes, and only for writing the marker that stops Spotlight indexing a card.

## Anything it does can be undone

A guiding principle rather than a feature list. Every change Chokepoint makes outside itself
...
Anything Chokepoint worked out rather than read carries **≈** and is drawn in violet: the SD
...
only means nothing is stopping Spotlight, so that reads `Spotlight not blocked` — Chokepoint
...
11.0, as low as Apple Silicon goes. Tested on macOS 11.7.11 / Intel (a 2015 MacBook)
and macOS 26 / Apple Silicon from the same universal binary; nothing between 10.14.4
and 11.7 has been tested.

`./test.sh` runs the suite **twice, once in each appearance**. Several colours are
