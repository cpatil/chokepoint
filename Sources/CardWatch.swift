...
/// What Chokepoint does by itself when a memory card is inserted.
///
...
/// is usually before Chokepoint is running - an app cannot notice an event it was not
/// there for.
///
...
enum CardWatch {
    ...
    enum Job: String, CaseIterable {
        ...
        var title: String {
            ...
            case .launchOnInsert: return "Open Chokepoint When a Card Is Inserted"
            }
        }
        ...
        var explanation: String {
            ...
            case .neverIndex:
                return "Writes a .metadata_never_index marker to a card as it mounts, "
                    ...
                    + "so this may need Chokepoint to be allowed under Privacy & Security."
            case .launchOnInsert:
                return "Opens Chokepoint as a card mounts, so the transfer is recorded from "
                    + "the first byte rather than from whenever you think to look.\n\n"
                    + "If Chokepoint is already running it simply comes forward - it allows "
    ...
    /// Records where this copy of Chokepoint lives, so the agent can open the same one
    ...
    static let label = "local.chokepoint.card-watch"
    ...
    enum Failure: LocalizedError {
        ...
        var errorDescription: String? {
            "This copy of Chokepoint is missing card-watch.sh, so the watcher cannot be "
