...
/// Two panels sharing one layout: what the colours mean, and how Chokepoint reaches the
...
final class LegendView: NSView {
    ...
    /// Every statement Chokepoint makes that is not a direct reading, with what it rests on
        ...
        [
            Statement(inferred: false, claim: "\u{201C}R 86 MB/s\u{201D}, \u{201C}2.36 TB used\u{201D}",
                      basis: "Read straight from the kernel's byte counters and the "
                           + "filesystem. Chokepoint only divides by the interval."),
            Statement(inferred: false, claim: "\u{201C}0% link utilization\u{201D}",
                      basis: "The rate divided by the link speed the system reported "
                           + "for that port. Both numbers are given to Chokepoint."),
...
enum LegendWindow {
    ...
    static func show(_ mode: LegendView.Mode) {
        ...
        window.title = mode == .colors ? "Color Key" : "How Chokepoint Infers"
...
final class RootView: NSView, NSSplitViewDelegate {
    ...
    /// information button answers "how does Chokepoint know that", which is a different
    ...
    private func setup() {
        ...
        for (button, what) in [(storageResort, "storage"), (networkResort, "network")] {
            ...
            button.toolTip = "Work out the \(what) order again.\n\n"
                + "\"Active first\" is decided when Chokepoint starts and then held, so rows "
        ...
        infoButton.title = ""
        infoButton.toolTip = "How Chokepoint infers: every conclusion it draws, and what "
        ...
        outerSplit.autosaveName = "ChokepointRows"
    ...
    func applyPaneLayout() {
        ...
        columnsSplit.autosaveName = stacked ? "ChokepointColumnsStacked" : "ChokepointColumns"
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        // Before anything reads either location: this app has been called Limen and
        // then Bottleneck, and a user's log, settings and watcher may still be filed
        // under either of those older names.
        Migration.run()
        ...
        window.center()
        window.setFrameAutosaveName("ChokepointWindow")
    ...
    @objc func revertCatalogue(_ sender: Any?) {
        ...
        if !Catalogue.usingDownloaded {
            ...
            done.informativeText = "Restart Chokepoint to use it."
    ...
    /// Everything Chokepoint remembers about how you like it, back to how it arrived.
    ...
    @objc func resetSettings(_ sender: Any?) {
        let alert = NSAlert()
        alert.messageText = "Put Chokepoint's settings back to their defaults?"
    ...
    private func offerUpdateIfDue() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) { [weak self] in
            ...
            alert.informativeText = "It has been a month since the last check. Chokepoint does not "
    ...
    @objc func checkForCatalogueUpdate(_ sender: Any?) {
        Catalogue.checkForUpdate { version, error in
            ...
            if let error = error {
                ...
                alert.informativeText = "Now at version \(version). Restart Chokepoint to use it."
            } else {
